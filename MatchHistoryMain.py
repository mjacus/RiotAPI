...
def main():
    ...
    summoner_id = Consts.SUMMONER_ID['ricofromjalisco']
    ...
    for i in range(ranked_games):
        ...
        if champ_type == 'DUO_CARRY':
            role[i] = 'ADC'
        elif champ_type == 'DUO_SUPPORT':
            role[i] = 'Support'
        else:
            role[i] = lane
        ...
        if num == 9:
            num = 0
            time.sleep(0.5)
            if i+10 >= ranked_games:
                api = RiotAPI('RANKED_SOLO_5x5', i, ranked_games, '82a7b066-b6f2-4e3f-8f15-9ee01aa68c22')
            else:
    ...
    with open('SoloQLog.csv', 'w') as csv_file:
